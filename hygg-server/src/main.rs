...
async fn acquire_lock(
  ...
  println!(
    "LOCK REQUEST: User {} is trying to acquire lock for progress {}",
    progress.user_id, progress.id
  );
  ...
  if exists.is_none() {
    println!(
      "PROGRESS INIT: Creating new progress entry for {} with id {}",
      progress.file_path, progress.id
    );
    sqlx::query("
  ...
  if let Some((_, holder, _)) =
    locks.iter().find(|(id, _, _)| *id == progress.id)
  {
    println!(
      "LOCK DENIED: Progress {} is already locked by {}",
      progress.id, holder
    );
    return Err((
  ...
  println!(
    "LOCK GRANTED: User {} acquired lock for progress {} until {}",
    progress.user_id, progress.id, expiry
  );
  ...
  println!(
    "PROGRESS LOADED: User {} acquired lock with position {}",
    updated_progress.user_id, updated_progress.position
  );
...
async fn release_lock(
  ...
  println!(
    "LOCK RELEASE: User {} is trying to release lock for progress {}",
    progress.user_id, progress.id
  );
  let mut locks = state.locks.lock().await;
  ...
  match lock_idx {
    Some(idx) => {
      ...
      locks.remove(idx);
      println!(
        "LOCK RELEASED: User {} released lock for progress {}",
        progress.user_id, progress.id
      );
    ...
    None => {
      println!(
        "LOCK RELEASE DENIED: User {} doesn't hold lock for progress {}",
        progress.user_id, progress.id
      );
      Err((
...
async fn update_progress(
  ...
  println!(
    "PROGRESS UPDATE: User {} is updating progress to position {}",
    progress.user_id, progress.position
  );
  ...
  if !locks
    .iter()
    .any(|(id, holder, _)| *id == progress.id && holder == &progress.user_id)
  {
    println!(
      "PROGRESS DENIED: User {} doesn't hold lock for progress {}",
      progress.user_id, progress.id
    );
    return Err((
      axum::http::StatusCode::FORBIDDEN,
  ...
  println!(
    "PROGRESS EVENT: Recording event id {} for progress {}",
    event.id, event.progress_id
  );
  ...
  println!(
    "PROGRESS UPDATED: User {} successfully updated position to {}",
    progress.user_id, progress.position
  );
  Ok(Json(progress))
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let app_state = AppState { db, locks: Arc::new(Mutex::new(Vec::new())) };
