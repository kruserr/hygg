...
pub fn get_help_text() -> Vec<String> {
  vec![
    ...
    "    :next, :continue       Next tutorial step (when completed)".to_string(),
    ...
    "    :!{cmd}                Execute shell command (opens in split view)".to_string(),
    "".to_string(),
    "  Split View Navigation:".to_string(),
    "    Ctrl+B j    Switch to bottom pane".to_string(),
    "    Ctrl+B k    Switch to top pane".to_string(),
    "    Ctrl+B x    Close current pane (when in command output)".to_string(),
    "    Ctrl+B q    Show pane numbers (for debugging)".to_string(),
