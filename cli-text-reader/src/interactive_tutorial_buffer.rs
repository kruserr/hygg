...
pub enum TutorialSuccessCondition {
  ...
  SearchTerm(String),
  SearchTermAndNavigated(String), // Requires both searching and navigating with n/N
  SearchBothDirections, // Requires using both forward (/) and backward (?) search
  YankOperation,
  CommandExecutedAndYanked(String), // Requires both command execution and yank
  CommandExecutedYankedAndPasted(String), // Requires command execution, yank, and paste
...
pub fn create_tutorial_buffer(
  ...
  if is_welcome {
    ...
    lines.push(format!("{} Type :next to begin the tutorial {}", 
      SetForegroundColor(Color::Green),
      ResetColor
    ));
  } else if is_credits {
    // Credits screen - show return message
    lines.push(format!("{}{} Type :next to return to your document {}", 
    ...
    lines.push(format!("{}{} ✓ Type :next to continue {}", 
      SetForegroundColor(Color::Green),
      SetAttribute(Attribute::Bold),
      ResetColor
    ));
    ...
    if step_completed {
      lines.push(format!("{}{} ✓ Great job! Type :next to continue {}", 
        SetForegroundColor(Color::Green),
        SetAttribute(Attribute::Bold),
        ResetColor
      ));
    } else {
      lines.push(format!("{} Complete the task above | :back to go back | :q to exit {}", 
        SetForegroundColor(Color::DarkGrey),
        ResetColor
      ));
    }
